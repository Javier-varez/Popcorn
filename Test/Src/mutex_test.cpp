...
TEST_F(MutexTest, CheckCanBeAcquired) {
  EXPECT_CALL(svc, SupervisorCall(OS::SyscallIdx::Lock));
  mutex->Lock();
...
TEST_F(MutexTest, NotAquiredUntilReleasedByOtherThread) {
  ...
  EXPECT_CALL(svc, SupervisorCall(OS::SyscallIdx::Wait)).Times(10);
  EXPECT_CALL(svc, SupervisorCall(OS::SyscallIdx::Wait)).WillOnce(Invoke(release));
  EXPECT_CALL(svc, SupervisorCall(OS::SyscallIdx::Lock));
  mutex->Lock();
  EXPECT_EQ(GetHeldFlag().test_and_set(), true);
